...
export const vault = storeConfig;
const redisAt = (v, command) => redis(command, v);
...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === "GET") {
      ...
      const blob = await redisAt(v, ["GET", `v:${id}`]);
    ...
    const saved = await redisAt(v, ["SET", `v:${id}`, blob, "NX", "EX", KEEP_SECONDS]);
    if (saved === "OK") return send(res, 200, { blob });
    const first = await redisAt(v, ["GET", `v:${id}`]);
