...
import { send, readBody, rateLimited, foreignOrigin, xai, modelFor, toMessages } from "./_xai.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    upstream = await xai({ model: modelFor(tier), messages, stream: true, temperature: 0.5, max_tokens: 4000 }, ctl.signal);
  } catch (err) {
  ...
  try {
    ...
    for await (const chunk of upstream.body) {
      ...
      while ((nl = buf.indexOf("\n")) >= 0) {
        ...
        if (c?.delta?.content) line({ delta: c.delta.content });
