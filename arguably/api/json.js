...
import { send, readBody, rateLimited, foreignOrigin, xai, modelFor, parseJsonReply } from "./_xai.js";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const pics = (Array.isArray(images) ? images : []).slice(0, 4).filter((u) => typeof u === "string" && /^data:image\/(jpeg|png);base64,/.test(u));
    const content = [
